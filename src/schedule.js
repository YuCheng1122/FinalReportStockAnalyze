...
// BWIBBU(每日本意比) --- 失敗
const updateBwibbu = schedule.scheduleJob('0 0 * * *', async () => {
  try {
...
// stock_day_all(每日股價資訊]) --- 失敗
const updateStockDayAll = schedule.scheduleJob('0 0 * * *', async () => {
  try {
...
// mi_index (每日產業指數)
const updateMiIndex = schedule.scheduleJob('0 0 * * *', async () => {
  try {
...
const updateTaiexData = schedule.scheduleJob('0 0 * * *', async () => {
  try {
